...
def create_qst_ui():
    ...
    # 第一行：选择清零点
    cmds.text(label='1 选择清零点', height=40)
    ...
    # 第三行：选择对称轴
    cmds.text(label='2 选择对称轴(一条边）', height=40)

    # 第四行：按钮，启用拓扑对称
    cmds.button(label='启用拓扑对称', command='QSTFuncs.symmetricModelling()')

    # 第五行：选择区域
    cmds.text(label='4 选择区域', height=40)
    ...
    # 第七行：按钮，执行
    cmds.button(label='执行', command='QSTFuncs.symSelection()')
